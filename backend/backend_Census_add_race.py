...
def getData(states, stateNumbers):
    for key in stateNumbers:
        url = (
            "https://api.census.gov/data/2018/acs/acs1?get=NAME,B02001_001E,B02001_002E,B02001_003E,B02001_004E,B02001_005E,B02001_006E,B02001_007E&for=congressional+district:*&in=state:"
            + stateNumbers[key]
        )
...
def generateSQL(states):
    print(
        "INSERT INTO staging.race_per_district(state, congressional_district, race, percentage) VALUES"
    )
...
def generateSQLUpdate(district):
    ...
    others = int(district[7]) / population * 100
    print(
        f"UPDATE application.districts SET white = {white:.2f} WHERE state_num='{stateNum}' and congressional_district='{districtNum}';"
    )
    print(
        f"UPDATE application.districts SET aa = {african:.2f} WHERE state_num='{stateNum}' and congressional_district='{districtNum}';"
    )
    print(
        f"UPDATE application.districts SET a_indian = {indian:.2f} WHERE state_num='{stateNum}' and congressional_district='{districtNum}';"
    )
    print(
        f"UPDATE application.districts SET asian = {asian:.2f} WHERE state_num='{stateNum}' and congressional_district='{districtNum}';"
    )
    print(
        f"UPDATE application.districts SET others = {others:.2f} WHERE state_num='{stateNum}' and congressional_district='{districtNum}';"
    )
    print(
        f"UPDATE application.districts SET hawaiian = {hawaiian:.2f} WHERE state_num='{stateNum}' and congressional_district='{districtNum}';"
    )
